...
class uEmuControlView(PluginForm):
    ...
    def PopulateForm(self):
        ...
        #add
        btnMark = QPushButton("_Mark_")
        btnPatch = QPushButton("_Patch_")
        #add end
        ...
        #add
        btnMark.clicked.connect(self.OnEmuMark)
        btnPatch.clicked.connect(self.OnEmuPatch)
        #add end
        ...
        #add
        hbox.addWidget(btnMark)
        hbox.addWidget(btnPatch)
        #add end
    ...
    def OnEmuMark(self, code=0):
        print('OnEmuMark')        
        items = self.owner.unicornEngine.get_mapped_memory()
        text1 = ''
        size = 0
        size = len(items)
        for i in range(size):
            text1 = '%s\n0x%x\t0x%x'%(text1, items[i][0], items[i][1])
        print text1
        text = '%s\nInput address: 0x%x(%d)'%(text1, items[0][0], items[0][0])
        self.address = AskLong(items[0][0], text)
        self.end = items[len(items)-1][1] if items[len(items)-1][1] < 0xffffffff else items[len(items)-2][1]
        text = '%s\nInput end: 0x%x(%d)'%(text1, self.end, self.end)    
        self.end = AskLong(self.end, text)
        self.memory = self.owner.unicornEngine.get_mapped_bytes(self.address, self.end-self.address+1)
        #for i in range(self.address, self.end+1):
        #    self.memory.append(self.owner.unicornEngine.get_mapped_bytes(i, 1)[0])
        print 'Mark memory len 0x%x'%(len(self.memory))

    def OnEmuPatch(self, form):
        print('OnEmuPatch')
        new_memory = bytearray()
        new_memory = self.owner.unicornEngine.get_mapped_bytes(self.address, self.end-self.address+1)
        #for i in range(self.address, self.end+1):
        #    new_memory.append(self.owner.unicornEngine.get_mapped_bytes(i, 1)[0])
        for i in range(len(new_memory)):
            if new_memory[i] != self.memory[i]:
                idc.PatchByte(self.address + i, new_memory[i])
        print 'Patch finish'
